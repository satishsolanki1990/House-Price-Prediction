...
def SSEplots(name,SSE,gamma):
    fig=plt.figure()
    fig.set_size_inches(16,9)
    x=[i for i in range(len(SSE))]
    ax=fig.add_subplot(1,1,1)
    ax.xaxis.set_tick_params(labelsize=20)
    ax.yaxis.set_tick_params(labelsize=20)
    ax.plot(x,(np.log(SSE)/np.log(100)),linewidth=3)
    plt.title(name+' SSE vs no. of itereation for Learning rate '+str(gamma),fontsize=20, fontweight='bold')
    plt.xlabel('No. of iteration', fontsize=20, fontweight='bold')
    plt.ylabel('log$_{100}$'+'(SSE)',fontsize=20, fontweight='bold')
    plt.savefig(name+'SSEplot_'+str(gamma)+'.png',dpi=1200)
    plt.show()
    return None
...
#    SSEplots('Training',temp[1],gamma)# plot and save SSE_train curves
...
#    features = temp[-1]
#
## part 1(b) Why did we choose 1e-5 as optimum learning rate.
## plot SSE_validation vs learning rate
#fig=plt.figure()
#fig.set_size_inches(16,9)
#x=np.log(gammas)/np.log(10)
#y=np.log(SSEs_validation)/np.log(100)
#ax=fig.add_subplot(1,1,1)
#ax.xaxis.set_tick_params(labelsize=20)
#ax.yaxis.set_tick_params(labelsize=20)
#ax.plot(x,y,linewidth=3)
#plt.title(' Validation SSE vs Learning rates',fontsize=20, fontweight='bold')
#plt.xlabel('log$_{10}$'+'(Learning rate)', fontsize=20, fontweight='bold')
#plt.ylabel('log$_{100}$'+'(SSE)',fontsize=20, fontweight='bold')
#plt.savefig('Validation_SSEplot.png',dpi=1200)
#plt.show()
#
#
...
for lambdA in lambdas:
    ...
    ws.append(temp[0])
## part 2: plots 
### plot SSE_validation vs lambda and SSE_training vs Lambda 
#fig=plt.figure()
#fig.set_size_inches(16,9)
#x=np.log(lambdas)/np.log(10)
#y_t=np.log(SSE_train)/np.log(100)
#y_v=np.log(SSE_validation)/np.log(100)
#ax=fig.add_subplot(1,1,1)
#ax.xaxis.set_tick_params(labelsize=20)
#ax.yaxis.set_tick_params(labelsize=20)
#ax.plot(x,y_t,x,y_v,linewidth=3)
#plt.title(' Training and Validation SSE vs lambda',fontsize=20, fontweight='bold')
#plt.xlabel('log$_{10}$'+'$(\lambda)$', fontsize=20, fontweight='bold')
#plt.ylabel('log$_{100}$'+'(SSE)',fontsize=20, fontweight='bold')
#plt.savefig('part2_SSEplot.png',dpi=1200)
#plt.legend(['Training SSE','Validation SSE'])
#plt.show()
##
...
#for part 2(d), figure in report :
fig = part2_w_table.plot(x='Features',y='Weights',kind='bar',rot=80).get_figure()
fig.set_size_inches(16,9)
fig.savefig('part1_best_w.png', dpi=1000)
#
...
#    SSEv.append(temp[3])
#    SSEplots('Training',temp[1],gamma)# plot and save SSE_train curves
#    SSEplots('Validation',temp[3],gamma)# plot and save SSE_train curves
#    
#    
